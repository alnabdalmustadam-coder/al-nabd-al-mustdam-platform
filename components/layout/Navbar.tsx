    ...
    <>
      ...
      >
        <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-12 flex items-center justify-between">
          ...
          {/* Desktop Links */}
          <div className="hidden xl:flex items-center gap-1 justify-center flex-1 mx-8 relative">
            {navLinks.map((link) => {
              ...
              if (link.hasMegaMenu) {
                return (
                  ...
                  >
                    <button
                      className={`flex items-center gap-1.5 px-3 py-6 -my-4 rounded-xl text-[15px] font-bold transition-all duration-300 ${
                    ...
                    <div 
                      className={`absolute top-full right-1/2 translate-x-1/2 pt-10 w-[800px] transition-all duration-300 transform origin-top ${
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[15px] font-bold transition-all duration-300 ${
                    isActive
                      ? "text-[#173A7C] bg-[#173A7C]/5"
                      : "text-slate-700 hover:text-[#173A7C] hover:bg-slate-50"
                  }`}
          ...
          <div className="hidden xl:flex items-center gap-3 shrink-0">
            <Link
              href="/auth/login"
              className="text-sm font-bold text-slate-700 hover:text-[#173A7C] transition-colors px-4 py-2"
            >
              تسجيل دخول
            </Link>
            <Button href="/auth/register" size="sm" className="hidden lg:flex">
              سجّل الآن
            </Button>
          </div>
          ...
          <button
            ...
            className={`xl:hidden p-2 text-slate-800 hover:text-[#173A7C]`}
            aria-label="القائمة"
          >
            ...
            {navLinks.map((link) => {
                ...
                return (
                  <div key={link.label} className="flex flex-col border-b border-slate-100 pb-2 mb-2">
                    <button
                      ...
                      className={`flex items-center justify-between px-4 py-3 rounded-xl text-base font-bold transition-all w-full ${
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-base font-bold transition-all mb-2 ${
                    isActive
                      ? "text-[#173A7C] bg-[#173A7C]/5"
                      : "text-slate-700 hover:text-[#173A7C] hover:bg-slate-50"
                  }`}
