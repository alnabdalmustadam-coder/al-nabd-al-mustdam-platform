...
import {
  Send, CheckCircle, Star, User, Phone, Briefcase, Building,
  Sparkles, ClipboardCheck, Users, MessageSquare, Globe,
  ChevronLeft, ChevronRight, Award, Mail, BookOpen, MonitorPlay, Lightbulb, UsersRound
} from "lucide-react";
...
const initialFormState: FormState = {
  ...
  futureCourses: ["", "", ""],
  source: "",
  nominees: [{ name: "", phone: "" }, { name: "", phone: "" }, { name: "", phone: "" }],
};
...
const steps = [
  ...
  { id: 2, label: "المركز", icon: Users },
  { id: 3, label: "التطبيق", icon: MonitorPlay },
  { id: 4, label: "إضافات", icon: Lightbulb },
];

export default function OnlineEvaluationPage() {
  ...
  const updateArrayField = (key: "futureCourses", index: number, value: string) => {
    setForm((prev) => {
  ...
  const progress = useMemo(() => {
    ...
    const hasCourse = form.courseName.trim() !== "";
    const total = ratingFields.length + 3;
    return Math.round(((filled + (hasName ? 1 : 0) + (hasPhone ? 1 : 0) + (hasCourse ? 1 : 0)) / total) * 100);
  }, [form]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.name.trim() || !form.phone.trim() || !form.courseName.trim()) {
      setActiveStep(0);
      window.scrollTo({ top: 200, behavior: "smooth" });
      showAlert("يرجى ملء اسم المتدرب ورقم الجوال واسم الدورة أولاً", "warning");
      return;
    }
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ type: "evaluation-online", ...form }),
      });
      if (res.ok) {
        setSubmitted(true);
  ...
  const goNext = () => {
    ...
    if (activeStep === 0 && (!form.name.trim() || !form.phone.trim() || !form.courseName.trim())) {
      showAlert("يرجى ملء الحقول الإجبارية (الاسم، الجوال، الدورة)", "warning");
      return;
    }
                ...
                {activeStep === 0 && (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                      ...
                      <div>
                        <label className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-2.5">
                          <Mail className="w-4 h-4 text-[#173A7C]" /> البريد الإلكتروني
                        </label>
                        <input
                          type="email" value={form.email} onChange={(e) => updateForm("email", e.target.value)}
                          className={`${inputClass} text-right`} dir="ltr" placeholder="example@email.com"
                        />
                      </div>
                      <div>
                        <label className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-2.5">
                          <BookOpen className="w-4 h-4 text-[#173A7C]" /> الدورة المراد تقييمها <span className="text-red-500">*</span>
                        </label>
                        <input
                          type="text" required value={form.courseName} onChange={(e) => updateForm("courseName", e.target.value)}
                          className={inputClass} placeholder="اسم الدورة التدريبية"
                        />
                      </div>
                    </div>
                  </motion.div>
                ...
                {activeStep === 1 && (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 mb-10 relative z-10">
                      ...
                      <div>
                        <h2 className="text-2xl font-black text-slate-900">تقييم البرنامج التدريبي والأداء التدريبي</h2>
                    ...
                    <div className="relative z-10 divide-y divide-slate-100/80">
                      <RatingRow label="القدرة على إيصال الأفكار وربطها بالواقع العملي" fieldName="trainerIdea" index={0} />
                      <RatingRow label="اكتساب مهارات ومعلومات جديدة" fieldName="trainerSkills" index={1} />
                      ...
                      <RatingRow label="المواد المرفقة مع الحقيبة من صوتيات او فيديوهات مناسبة ومفيدة ومرتبطة بالموضوع" fieldName="trainerMaterials" index={4} />
                    </div>
                  </motion.div>
                )}
                ...
                {activeStep === 2 && (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 mb-10 relative z-10">
                      ...
                      <div>
                        <h2 className="text-2xl font-black text-slate-900">تقييم فريق الإشراف والاستقبال</h2>
                        <p className="text-sm text-slate-400 font-medium mt-1">المركز وفريق الدعم</p>
                      </div>
                    </div>

                    <div className="relative z-10 divide-y divide-slate-100/80">
                      <RatingRow label="سرعة التسجيل واجراءات الدخول للدورة الالكترونية" fieldName="centerRegistration" index={0} />
                      <RatingRow label="التعامل بأسلوب راقي ومتفهم من قبل فريق الاشراف" fieldName="centerTreatment" index={1} />
                ...
                {activeStep === 3 && (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 mb-10 relative z-10">
                      ...
                      <div>
                        <h2 className="text-2xl font-black text-slate-900">تقييم مستوى التطبيق او القاعة الالكترونية</h2>
                        <p className="text-sm text-slate-400 font-medium mt-1">المنصة التعليمية والتجربة التقنية</p>
                      </div>
                    </div>

                    <div className="relative z-10 divide-y divide-slate-100/80">
                      <RatingRow label="التطبيق تفاعلي بشكل مناسب" fieldName="appInteractive" index={0} />
                      <RatingRow label="سهولة الاستخدام للتطبيق" fieldName="appUsability" index={1} />
                      <RatingRow label="وضوح الصوت ووضوح العرض" fieldName="appClarity" index={2} />
                    </div>
                  </motion.div>
                )}

                {/* ─── Step 4: Notes, Future Courses, Source, Nominees ─── */}
                {activeStep === 4 && (
                  <motion.div
                    key="step-4"
                    initial={{ opacity: 0, x: 30 }}
                  ...
                  >
                    ...
                    <div className="relative z-10 flex flex-col gap-10">
                      ...
                      <div>
                        <div className="flex items-center gap-4 mb-6">
                          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#173A7C] to-[#2E5EA6] flex items-center justify-center shadow-lg shadow-[#173A7C]/20">
                            <BookOpen className="w-7 h-7 text-white" />
                          </div>
                          <div>
                            <h2 className="text-2xl font-black text-slate-900">الدورات المستقبلية</h2>
                            <p className="text-sm text-slate-400 font-medium mt-1">ما هي الدورات التي ترغب بالإشتراك بها خلال الفترة القادمة؟</p>
                          </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {[0, 1, 2].map((idx) => (
                            <div key={idx} className="relative">
                              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm">-{idx + 1}</span>
                              <input
                                type="text"
                                value={form.futureCourses[idx]}
                                onChange={(e) => updateArrayField("futureCourses", idx, e.target.value)}
                      ...
                      <div>
                        <div className="flex items-center gap-4 mb-6">
                          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#5CB07C] to-emerald-600 flex items-center justify-center shadow-lg shadow-emerald-500/20">
                            <UsersRound className="w-7 h-7 text-white" />
                          </div>
                          <div>
                            <h2 className="text-2xl font-black text-slate-900">جهات الاتصال المقترحة</h2>
                            <p className="text-sm text-slate-400 font-medium mt-1">أذكر أسماء أشخاص ترشحهم لحضور هذه الدورة مستقبلاً</p>
                          </div>
                        </div>
                        
                        <div className="space-y-4">
                          {[0, 1, 2].map((idx) => (
                            <div key={idx} className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                              <input
                                type="text" value={form.nominees[idx].name} onChange={(e) => updateNominee(idx, "name", e.target.value)}
                                className={inputClass} placeholder="الاسم"
                              />
                              <input
                                type="tel" dir="ltr" value={form.nominees[idx].phone} onChange={(e) => updateNominee(idx, "phone", e.target.value)}
                                className={`${inputClass} text-right`} placeholder="رقم الجوال"
                              />
                            </div>
                          ))}
                        </div>
