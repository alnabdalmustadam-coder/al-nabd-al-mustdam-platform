...
function CheckoutContent() {
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#070b19]">
        ...
        <p className="mt-6 text-slate-400 font-bold text-lg animate-pulse">جاري التحقق من الحساب والأمان...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen pt-28 pb-20 bg-[#070b19] relative overflow-hidden text-white font-sans">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Navigation / Header */}
        <div className="mb-10 flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-white/5 pb-6">
          <div className="text-right">
            ...
            <h1 className="text-3xl sm:text-4xl font-black text-white">إتمام <span className="text-[#5CB07C]">الطلب والاشتراك</span></h1>
          </div>
          <Link 
            href={`/courses/${course.slug}`} 
            className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 text-sm font-bold transition-all text-slate-300 hover:text-white"
          >
        ...
        {errorMessage && (
          <motion.div 
            ...
            className="mb-6 p-4 rounded-2xl bg-red-950/40 border border-red-900/30 text-red-300 text-sm font-bold flex items-center gap-3"
          >
        ...
        <form onSubmit={handleCheckout} className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-[28px] p-8 shadow-2xl relative overflow-hidden">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/5 pb-5 mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-white">بيانات الاشتراك والحساب</h3>
                    <p className="text-xs text-slate-400 font-medium">تم التحقق من هويتك بنجاح ومزامنتها مع الطلب.</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="bg-white/5 border border-white/5 p-4 rounded-2xl">
                  ...
                  <span className="text-base font-black text-slate-200">{userName || "-"}</span>
                </div>
                <div className="bg-white/5 border border-white/5 p-4 rounded-2xl">
                  <span className="text-xs text-slate-400 font-bold block mb-1">البريد الإلكتروني</span>
                  <span className="text-base font-black text-slate-200 truncate block" dir="ltr">{email || "-"}</span>
                </div>
                ...
                {!phone ? (
                  <div className="sm:col-span-2 bg-[#5CB07C]/5 border border-[#5CB07C]/20 p-5 rounded-2xl">
                    ...
                    <input
                      ...
                      required
                      className="w-full px-5 py-3.5 rounded-xl bg-[#070b19]/80 border border-white/10 text-white placeholder:text-slate-500 focus:border-[#5CB07C] focus:ring-1 focus:ring-[#5CB07C] outline-none text-base transition-all"
                      placeholder="+966 5X XXX XXXX"
                      dir="ltr"
                    />
                    <p className="text-xs text-slate-400 mt-2 font-medium">ملاحظة: هذا الحقل ضروري لاعتماد شهادات NELC الحكومية وتلقي إشعارات الدورة.</p>
                  </div>
                ) : (
                  <div className="sm:col-span-2 bg-white/5 border border-white/5 p-4 rounded-2xl">
                    <span className="text-xs text-slate-400 font-bold block mb-1">رقم الجوال</span>
                    <span className="text-base font-black text-slate-200" dir="ltr">{phone}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-[28px] p-8 shadow-2xl">
              <h2 className="text-xl font-black text-white mb-6 border-b border-white/5 pb-4 flex items-center gap-2">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-5 rounded-2xl border text-center transition-all duration-300 cursor-pointer flex flex-col items-center justify-center gap-2 group ${
                      paymentMethod === m.key
                        ? "bg-[#5CB07C]/10 border-[#5CB07C] text-white shadow-lg shadow-[#5CB07C]/10 scale-[1.02]"
                        : "bg-white/5 border-white/10 text-slate-400 hover:border-white/20 hover:bg-white/10 hover:text-white"
                    }`}
                  >
              ...
              <AnimatePresence mode="wait">
                {paymentMethod === "card" && (
                  ...
                  >
                    ...
                    <div className="relative mx-auto max-w-sm h-48 rounded-2xl bg-gradient-to-tr from-[#1b4332] to-[#2d6a4f] border border-white/10 p-6 flex flex-col justify-between shadow-2xl overflow-hidden mb-6">
                      <div className="absolute -top-10 -right-10 w-40 h-40 bg-[#5CB07C]/10 rounded-full blur-2xl" />
                      ...
                      <div className="flex justify-between items-end relative z-10 font-sans">
                        <div>
                          <span className="text-[10px] text-slate-400 block uppercase font-bold">صاحب البطاقة</span>
                          <span className="text-sm font-bold text-white truncate max-w-[200px] block">{userName || "اسم المتدرب"}</span>
                        </div>
                        <div>
                          <span className="text-[10px] text-slate-400 block uppercase font-bold">الانتهاء</span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                      <div className="sm:col-span-3">
                        <label className="text-sm font-bold text-slate-300 block mb-2">رقم البطاقة</label>
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full pr-12 pl-4 py-3.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-slate-600 focus:border-[#5CB07C] focus:ring-1 focus:ring-[#5CB07C] outline-none text-base font-mono"
                      ...
                      <div className="sm:col-span-2">
                        <label className="text-sm font-bold text-slate-300 block mb-2">تاريخ الانتهاء</label>
                        <input
                          type="text"
                          maxLength={5}
                          value={cardExpiry}
                          ...
                          required={paymentMethod === "card"}
                          className="w-full px-4 py-3.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-slate-600 focus:border-[#5CB07C] focus:ring-1 focus:ring-[#5CB07C] outline-none text-base font-mono"
                          placeholder="MM/YY"
                          dir="ltr"
                        />
                      </div>
                      <div>
                        <label className="text-sm font-bold text-slate-300 block mb-2">الرمز السري (CVV)</label>
                        <input
                          type="password"
                          maxLength={3}
                          value={cardCvv}
                ...
                {paymentMethod === "mada" && (
                  ...
                  >
                    ...
                    <h4 className="text-base font-bold mb-1">دفع فوري عبر شبكة مدى الوطنية</h4>
                    <p className="text-xs text-slate-400">سيتم معالجة عملية الدفع بأمان مباشرة من حسابك البنكي.</p>
                  </motion.div>
                )}
                ...
                {paymentMethod === "apple" && (
                  ...
                  >
                    ...
                    <h4 className="text-base font-bold mb-1">دفع بلمسة واحدة باستخدام Apple Pay</h4>
                    <p className="text-xs text-slate-400">تأكيد عملية الشراء فوري باستخدام بصمة الوجه أو الاصبع.</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
            ...
            {/* Security SSL seal */}
            <div className="flex items-center justify-center gap-2 text-xs font-semibold text-slate-400 py-2 bg-white/5 border border-white/5 rounded-2xl">
          ...
          <aside className="lg:col-span-1">
            <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-[32px] p-8 shadow-2xl sticky top-28 space-y-6">
              <h2 className="text-lg font-black text-white border-b border-white/5 pb-4">ملخص الاشتراك</h2>
              ...
              <div className="p-4 rounded-2xl bg-white/5 border border-white/5 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-bold text-white leading-normal truncate">{course.title}</h4>
                  <p className="text-xs font-semibold text-slate-400 mt-1">{course.duration} · {course.lessonsCount} درس</p>
                </div>
              </div>

              {/* Pricing Breakdowns */}
              <div className="space-y-4 pt-4 border-t border-white/5 font-medium">
                ...
                <div className="flex items-center justify-between text-lg font-black pt-4 border-t border-white/5 text-white">
