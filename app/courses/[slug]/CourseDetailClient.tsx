...
import type { Course, CurriculumSection } from "@/types";
...
export default function CourseDetailClient({ course, relatedCourses }: CourseDetailClientProps) {
  ...
  const fullDescription = (course.longDescription || course.description || "").trim();
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-[family-name:var(--font-cairo)] text-slate-900 selection:bg-[#5CB07C] selection:text-white" dir="rtl">
      ...
      <section className="relative min-h-[85vh] lg:min-h-[90vh] flex flex-col justify-center pt-20 sm:pt-28 pb-16 overflow-hidden bg-gradient-to-b from-slate-100/90 via-slate-50 to-slate-100/60 border-b border-slate-200/80">
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-[0.05] pointer-events-none overflow-hidden z-0">
          <Image src="/bg.webp" alt="" fill sizes="100vw" className="object-cover" />
        </div>
                  ...
                  <div className="flex items-center justify-center gap-2 sm:justify-start">
                    {!isEnrolled && (
                      <button
                        ...
                        aria-label={inCart ? "موجودة في السلة" : "إضافة إلى السلة"}
                      >
                    ...
                    <button
                      onClick={handleShare}
                      className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-slate-100 hover:bg-slate-200 text-slate-600 transition-all cursor-pointer"
                      title="مشاركة رابط الدورة"
                      aria-label="مشاركة رابط الدورة"
                    >
                              ...
                              {isOpen && (
                                ...
                                >
                                  {section.lessons && section.lessons.length > 0 ? (
                                    section.lessons.map((lesson, lIdx: number) => {
                                      const lessonTitle = typeof lesson === "string" ? lesson : lesson.title;
                                      const lessonDuration =
                                        typeof lesson === "string" ? section.duration : lesson.duration || section.duration;

                                      return (
                                        <div
                                          key={typeof lesson === "string" ? lIdx : lesson.id || lIdx}
                                        ...
                                        >
                                          <div className="flex items-center gap-2.5">
                                            ...
                                            <span>
                                              {lessonTitle.length > 2
                                                ? lessonTitle
                                                : `الدرس ${lIdx + 1}: التطبيق العملي والشرح`}
                                            </span>
                                          </div>
                                          <span className="text-xs text-slate-400 font-normal">
                                            {lessonDuration || "20 دقيقة"}
                                          </span>
                                        </div>
                                      );
                                    })
